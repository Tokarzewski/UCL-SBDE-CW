...
class EditFractionRadiantOfLightingAndEquipment < OpenStudio::Ruleset::ModelUserScript
  ...
  # human readable name
  def name
    return "Edit Fraction Radiant of Lighting, Equipment and People"
  end

  # human readable description
  def description
    return "This measure replaces the 'Fraction Radiant' of all lights, equipment and people in the model with values that you specify. This is useful for thermal comfort studies where the percentage of heat transferred to the air is important."
  end
  ...
  def run(model, runner, user_arguments)
	...
	peopleFract = runner.getStringArgumentValue("peopleFractRad", user_arguments).to_f
	...
	if peopleFract < 0 or peopleFract > 1
      runner.registerError("People Fraction Radiant must be between 0 and 1.")
      return false
    end	
	...
	space_types.each do |space_type|
		...
		people.each do |person|
			pdef = model.getPeople(person.handle).get
			finalDef = model.getPeopleDefinition(pdef.peopleDefinition.handle).get
			finalDef.setFractionRadiant(peopleFract)
			peopleCount += 1
		end
	end
	...
	runner.registerFinalCondition("The building finished with #{peopleCount} people definitions with their fraction radiant set to #{peopleFract}.")
    return true
